...
std::ostream& operator << (std::ostream& dest, const __int128& value) noexcept;
std::ostream& operator << (std::ostream& dest, const unsigned __int128& value) noexcept;
std::istream& operator >> (std::istream& src, unsigned __int128& value) noexcept;
std::istream& operator >> (std::istream& src, __int128& value) noexcept;
...
public:
    ...
    template <typename sm = stream_mixin> engine(itype state, typename sm::stream_state stream_seed)
    ...
    template<typename SeedSeq> engine(SeedSeq&& seedSeq, typename std::enable_if <
...
public:
    ...
    template <typename bc = baseclass> extended(const result_type* data, state_type seed,
    ...
    template <typename bc = baseclass> extended(state_type seed, typename bc::stream_state stream_seed)
        : baseclass(seed, stream_seed)
    {
        selfinit();
    }
...
public:
    ...
    template < typename SeedSeq, typename = typename std::enable_if <
                   ...
                   && !std::is_convertible<SeedSeq, extended>::value >::type > extended(SeedSeq && seedSeq)
        : baseclass(seedSeq)
    {
